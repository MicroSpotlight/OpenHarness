const copy = {
  zh: {
    ...
    pluginsKicker: "插件系统",
    pluginsTitle: "从发现到激活，都在 App 里完成。",
    pluginsBody: "在“设置 → 插件 → 发现插件”中搜索目录、查看兼容性和发布者信息，并安装或升级到目录锁定的精确版本。",
    pluginsCatalogLink: "打开 OpenHarness 插件市场",
    pluginsFlowLabel: "受管插件安装与升级流程",
    pluginsConsoleTitle: "插件操作",
    pluginsManagedStatus: "本机受管",
    pluginsDiscoverTitle: "发现",
    pluginsDiscoverBody: "按名称、分类和能力搜索经过 Schema 校验的插件目录。",
    pluginsVerifyTitle: "校验",
    pluginsVerifyBody: "Host 重新校验目录 revision、身份、精确版本或 Git commit。",
    pluginsInstallTitle: "安装或升级",
    pluginsInstallBody: "使用 App 内置 Node.js、DSH 与 pnpm 执行，可查看进度并取消。",
    pluginsActivateTitle: "验证并激活",
    pluginsActivateBody: "验证安装结果；可以热激活时立即生效，否则由 App 受管重启。",
    pluginsIdentityTitle: "精确身份",
    pluginsIdentityBody: "只把严格匹配的包认作已安装，冲突和不确定状态会明确显示。",
    pluginsSafetyTitle: "受控执行",
    pluginsSafetyBody: "浏览器不提交命令或安装源，所有变更由本机 Host 根据目录重新推导。",
    pluginsRecoveryTitle: "可取消、可恢复",
    pluginsRecoveryBody: "单操作互斥、完整进程树取消、超时限制和失败后的 profile 元数据恢复。",
    ...
    faqPluginQuestion: "插件如何安装和更新？",
    faqPluginAnswer: "在“设置 → 插件 → 发现插件”中操作。Find Plugin Host 会校验目录并管理安装事务，OpenHarness App 使用内置 Node.js、DSH 和 pnpm 执行变更；不需要系统安装 Node.js 或 pnpm。",
  ...
  en: {
    ...
    factHarnessValue: "Discover, install, and update plugins",
    ...
    pluginsKicker: "Plugin system",
    pluginsTitle: "From discovery to activation, all inside the app.",
    pluginsBody: "Open Settings → Plugins → Discover to search the catalog, review compatibility and publisher details, and install or upgrade to the exact catalog-pinned version.",
    pluginsCatalogLink: "Open the OpenHarness plugin marketplace",
    pluginsFlowLabel: "Managed plugin installation and upgrade flow",
    pluginsConsoleTitle: "PLUGIN OPERATION",
    pluginsManagedStatus: "Managed locally",
    pluginsDiscoverTitle: "Discover",
    pluginsDiscoverBody: "Search a schema-validated catalog by name, category, and capability.",
    pluginsVerifyTitle: "Verify",
    pluginsVerifyBody: "The Host revalidates catalog revision, identity, exact version, or Git commit.",
    pluginsInstallTitle: "Install or upgrade",
    pluginsInstallBody: "Run with the Node.js, DSH, and pnpm bundled in the app, with live progress and cancellation.",
    pluginsActivateTitle: "Validate and activate",
    pluginsActivateBody: "Verify the result and activate live when safe; otherwise restart through the app supervisor.",
    pluginsIdentityTitle: "Exact identity",
    pluginsIdentityBody: "Only strict package matches count as installed; conflicts and unknown states stay explicit.",
    pluginsSafetyTitle: "Controlled execution",
    pluginsSafetyBody: "The browser submits neither commands nor install sources. The local Host derives every change from the catalog.",
    pluginsRecoveryTitle: "Cancelable and recoverable",
    pluginsRecoveryBody: "One mutation at a time, full process-tree cancellation, bounded time, and profile metadata recovery on failure.",
    ...
    faqPluginQuestion: "How are plugins installed and updated?",
    faqPluginAnswer: "Use Settings → Plugins → Discover. The Find Plugin Host validates the catalog and owns the installation transaction, while OpenHarness runs it with the bundled Node.js, DSH, and pnpm environment. No system Node.js or pnpm installation is required.",
